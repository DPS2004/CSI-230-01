...
# b) Generate failed logins and test
    logline=$(cat "$authfile" | grep "unix_chkpwd" | grep "password check failed")
    dateAndUser=$(echo "$logline" | cut -d' ' -f1,2,11 | tr -d '\.()')
    echo "$dateAndUser"
...
echo "To: daphne.sophinos@mymail.champlain.edu" > emailform.txt
echo "Subject: Logins" >> emailform.txt
getLogins >> emailform.txt
cat emailform.txt | ssmtp daphne.sophinos@mymail.champlain.edu
